...
# This harness is about the OUTPUT-TREE LOCK, not about disk, and it runs on
# hosted runners whose free space is not this suite's business — a lock test
# that goes red because the runner is 70% full is a test that gets disabled.
# Pin the floor to 0 MiB: it is a threshold, not a skip, so the preflight still
...
make_worktree() {
  ...
  # rots silently the moment connected-test.sh sources a new library: every
  # fixture then dies at `source: No such file or directory` and every case in
  # this file reports its own vacuous-pass guard instead of the property it
  # exists to check. avd-pool-test.sh learned this in #1853 (its list predated
  # agents-pool.sh/scope-run.sh and the whole suite had been failing on main);
  # this list predated disk-preflight.sh (#1989) and broke the same way.
