 ...
 * [ConnectivityManager.NetworkCallback]. Its consumers call [refresh]
 * on-tap from the foreground, so a one-shot read against the platform state
 * is all that is needed — D21 (no-background-work) compliant by
 * construction. The reconnect ladder uses its own foreground signal; this
 * class never feeds it.
 *
...
class ConnectivityObserver @Inject constructor(
     ...
     */
    override fun refresh(): Boolean = currentNetwork()
