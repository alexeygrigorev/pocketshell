 ...
 * strings live here.
 *
 * [hostsScreen], [connectViewModel], [treeScreen] and [sessionScreen] are
 * seams, not feature flags: the real host list, connect gate, session tree and
 * terminal resolve their ViewModels through `hiltViewModel()`, which needs a
 * Hilt-managed Activity, so a plain Robolectric `createComposeRule()`
