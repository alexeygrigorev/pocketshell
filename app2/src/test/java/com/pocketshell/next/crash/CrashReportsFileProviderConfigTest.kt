...
/**
 * Adapted from the old app's `CrashReportsFileProviderConfigTest` (rewrite
 * task P-10): asserts app2's manifest declares the `.fileprovider` authority
 * that `CrashReportsScreen`'s "Share all reports" content-URI hand-off needs,
 * pointed at the `report-archives/` cache-path the "Share all" archive is
 * written under (`REPORT_ARCHIVES_CACHE_DIR` in `CrashReportsViewModel.kt`).
 */
class CrashReportsFileProviderConfigTest {

    @Test
    fun manifestFileProviderUsesExportPathsResource() {
        val manifest = projectRoot().resolve("app2/src/main/AndroidManifest.xml").toFile()
        ...
        val fileProviderUsesExportPaths = (0 until providers.length).any { providerIndex ->
            ...
            (0 until metadata.length).any { metadataIndex ->
                ...
                item.getAttributeNS(ANDROID_NS, "resource") == "@xml/crash_report_export_paths"
            }
        }
    ...
    fun fileProviderExposesReportArchivesCacheDirectory() {
        val pathsXml = projectRoot().resolve("app2/src/main/res/xml/crash_report_export_paths.xml").toFile()
        val document = DocumentBuilderFactory.newInstance()
            .newDocumentBuilder()
            .parse(pathsXml)
