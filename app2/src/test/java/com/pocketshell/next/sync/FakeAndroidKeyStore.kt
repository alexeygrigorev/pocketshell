 ...
 *
 * Copied rather than shared (issue #2633): `:shared:core-voice` and
 * `:shared:core-assistant` each carry their own for the same reason — a test
 * source set is not published, and adding a `testFixtures` variant to a
 * shared module to export twenty lines of JCA shim would be a bigger dependency
