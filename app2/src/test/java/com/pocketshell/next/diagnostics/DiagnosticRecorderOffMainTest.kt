...
/**
 * Ported (behaviour unchanged, no settings-repository dependency) from the
 * old app's `DiagnosticRecorderOffMainTest` (rewrite task P-10, regression
 * proof for old-app issue #1124): the highest-impact cold-launch freeze
 * found in that hunt. [DiagnosticRecorder] is constructed during
 * `App.onCreate` Hilt injection on the **Main thread**. The naive
 * implementation seeds `AtomicLong(store.lastSequence())` synchronously in
 * `<init>`, and [DiagnosticLogStore.lastSequence] does a **full synchronous
 * `readLines()`** of the unbounded, ever-growing diagnostics JSONL — so the
 * launch cost *grows with usage*.
 *
 * Reproduce-first (D33/G10): the LOAD-BEARING assertion is that the JSONL
 ...
 * fresh instance over an existing JSONL) keep counting up from the persisted
...
class DiagnosticRecorderOffMainTest {
    ...
    /**
     * LOAD-BEARING: the unbounded diagnostics JSONL `lastSequence()` read
     * must NOT run on the thread that constructs the recorder (the Main
     * thread during `App.onCreate` Hilt injection in production).
     */
    @Test
    fun `lastSequence read does not run on the constructing thread`() {
        ...
        assertNotEquals(
            "the diagnostics JSONL lastSequence() read must run off the " +
                "constructing (Main) thread, not on it. " +
    ...
    /**
     * Monotonic-sequence contract preserved (G2): a fresh recorder instance
     * over an existing JSONL (a process restart) continues numbering from
     * the persisted high-water mark seeded off-main, never restarting at 1.
     */
    @Test
    fun `sequence resumes from persisted high-water mark after off-main seed`() = runTest {
        ...
        assertEquals(
            "the new event must resume from the persisted high-water mark",
            4L,
    ...
    /**
     * A fresh-install recorder (no prior JSONL) still numbers from 1 after
     * the off-main seed — the empty-file / missing-data case (G2).
     */
    @Test
