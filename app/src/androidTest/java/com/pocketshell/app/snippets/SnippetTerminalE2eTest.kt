...
/**
 * Emulator + Docker regression coverage for issues #60 and #187.
 *
 * Drives the UI tap surfaces (row body for the legacy smart-default
 * path, and the explicit `Send` / `Send + ↵` chips added by #187)
 * instead of calling the ViewModel directly: each tap must route into
...
class SnippetTerminalE2eTest {
    ...
    /**
     * Issue #187: tapping the explicit `Send + ↵` chip on a command
     * snippet must press Enter so the body executes on the remote shell.
     *
    ...
    fun tappingSendWithEnterChip_executesCommandSnippet() = runBlocking {
        ...
        try {
            compose.setContent {
                PocketShellTheme(mode = PocketShellThemeMode.Dark) {
                    SnippetPickerContent(
                        ...
                        query = "",
                        onQueryChange = {},
                        // Row-body smart-default tap is left to the
                        // legacy ViewModel path; the explicit
                        // [onSnippetSend] callback (issue #187) is the
                        // subject under test.
                        onSnippetTap = viewModel::onSnippetPicked,
                        onSnippetSend = { picked, withEnter ->
                            // Production wiring (issue #187): the real
                            // SessionScreen call site invokes
            ...
            // Tap the explicit Send + ↵ chip (not the row body).
    ...
    /**
     * Issue #187: tapping the plain `Send` chip on a prompt snippet
     * must NOT press Enter. The snippet body is parked on the shell's
     * input line but never executed; the user can continue typing or
     * dismiss the line manually.
     *
