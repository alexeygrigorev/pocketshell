...
Two aplexer version-trap lessons, both learned the expensive way: a locally-built `a` and a published wheel can BOTH self-report the same `--version` while differing by 143 commits, so pin behaviour in a test (`tools/pocketshell/tests/test_aplexer_contract.py`), never a version string. And most of aplexer's release machinery postdates a green release, so it has never run - budget for the release gate itself being broken, not just the code.
