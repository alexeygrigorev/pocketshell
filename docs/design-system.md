...
| Type | [`Type.kt`](../shared/ui-kit/src/main/java/com/pocketshell/uikit/theme/Type.kt) overrides `headlineSmall` 20sp, `titleMedium` 16sp, `bodyMedium` 14sp, `labelSmall` 11sp, mirrored by `PocketShellType`'s `screen`/`title`/`body`/`metadata`/`label`. `PocketShellType` also adds `bodyDense` 13sp, `bodyMono` 13sp, and `labelMono` 11sp. | Screen code still has raw `10.sp`, `12.sp`, `13.sp`, `15.sp`, and custom line heights, especially conversation, markdown, keybar, terminal chrome, and legacy local rows. | This table MIRRORS [`design-language.md`](design-language.md) § Type, which is the source of truth — change that document first. Use M3 slots for normal chrome, `PocketShellType.bodyDense` for compact rows, `bodyMono` for paths/commands/IDs, and `labelMono` for compact code labels. `QuietThemeTokenTest` fails the build if the code drifts from these numbers (#2630). |
| Spacing | [`Spacing.kt`](../shared/ui-kit/src/main/java/com/pocketshell/uikit/theme/Spacing.kt) codifies `xs` 4dp, `sm` 8dp, `md` 12dp, `lg` 16dp, `xl` 20dp, `xxl` 24dp, `section` 32dp, plus the density tokens below. | Raw `.dp` literals are widespread in screens and components, including off-grid values like 2, 5, 6, 10, 14, 28, 30, and 38 where component geometry needs explicit tokens. | Keep the base spacing scale small. Add component-specific geometry tokens only when a pattern repeats across surfaces. |
