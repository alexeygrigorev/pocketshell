...
Ratchet-style oversized-file guard over tracked, first-party files:
  ...
  - vendor/generated/worktree directories, shared/core-terminal and
    docs/design-kit (vendored design handoff) are exempt.
...
path_is_exempt() {
  ...
  case "$path" in
    shared/core-terminal|shared/core-terminal/*)
      return 0
      ;;
    docs/design-kit|docs/design-kit/*)
      # Vendored design handoff (39eae7356). Its tokens.css, Compose theme,
      # icon registry and 81-frame fixture catalog are GENERATED from
      # tokens.json/catalog.json by the kit's own build scripts, which do not
      # live in this repo -- so the ratchet has nothing actionable to say
      # about their size and "split the file" is not available to us.
      return 0
      ;;
...
write_baseline() {
  local root="$1" file="$2"
  {
    ...
    echo "# Exemptions: vendor/generated/worktree dirs, shared/core-terminal, docs/design-kit."
