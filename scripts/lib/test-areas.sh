...
pocketshell_test_areas_load() {
  ...
  POCKETSHELL_TA_PROD_PKG_AREA=(); POCKETSHELL_TA_PROD_PKG_SHARED=()
...
#
# HOST-CLI WIRE SEAM: DELETED (issue #2643).
#
# The host CLI moved to PocketShell-io/pocketshell-cli, and the wire-seam
# machinery that mapped a producer-side change onto the Kotlin wire tests
# (invoker/consumer marking, the live-Click vocabulary reader, the
# host-cli area's per-end floors, the I9 lockstep pins) was deleted with it —
# the D22 rule that deletes a guard's subject deletes the guard. The
# PROTECTION survives in the two places that now own it: a bump of
# tests/docker/fixture-pins.txt (the pinned producer wheel the Docker
# fixtures install) force-fulls the whole run via the manifest's
# `full tests/docker/*` row, and the cross-repo contract tests live in the
# CLI repo's own CI. The Kotlin wire tests themselves are untouched: they run
# on changes to their own packages, exactly as any other test.
#
...
pocketshell_test_areas_build_index() {
  ...
  # 2. THE HOST-CLI WIRE SEAM was deleted with the producer it marked
  #    (issue #2643) — see the block comment above the declarations.
  ...
  if [[ "${#test_files[@]}" -gt 0 ]]; then
    ...
    for p in "${!POCKETSHELL_TA_PROD_PKG_AREA[@]}"; do
      awk_map+="M	$p	${POCKETSHELL_TA_PROD_PKG_AREA[$p]}"$'\n'
    done
...
# compiles against, plus its own area. Falls back to the union over its test
# package for a
