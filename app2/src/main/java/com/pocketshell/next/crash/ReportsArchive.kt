 ...
 * `com.pocketshell.app.crash.ReportsArchive` (rewrite task P-10).
 *
 * Packs all local crash reports into a single zip archive for the
 * "Share all reports" action. Pure file-system logic, unit-testable without
 * an emulator: it enumerates the report files, writes them into one zip, and
 * returns the archive [File]. The screen exposes that file through
 * FileProvider and Android's share sheet.
 *
