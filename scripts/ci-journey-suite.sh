...
JOURNEY_CLASSES=(
  ...
  "com.pocketshell.app.proof.signals.TerminalViewportCaptureTest"  # #2135 D32: the shared terminal-viewport capture must hard-fail on a 0x0 / missing view instead of silently writing nothing — the artifact was absent exactly when the run was broken
  "com.pocketshell.app.proof.signals.SessionCaptureContractTest"  # #2297 D32: semantic-before-bitmap ordering, recovery-card/session-frame routing, explicit visible tags, and session-route stability
  ...
  "$FQCN_PREFIX.Issue2463BackgroundHostKeyTrustNoNavJourneyE2eTest"  # #2463 D33/G2/G10: a BACKGROUND host-key verification failure (v19 migration leaves trustedHostKeySha256 null; cold-launch reprobe) must annotate the host card, never navigate the user off the Hosts list; an explicit tap on the same host must still reach the Trust screen
  ...
  "com.pocketshell.app.tmux.Issue2155ConversationRebindsAfterInSessionRelaunchDockerTest"  # #2155 D33/G10: after an in-session relaunch the Conversation must rebind to the NEW recorded transcript. The host @ps_agent_source is already correct; the client has to observe the generation bump on the existing list-panes reconcile (cwd/command/tty often unchanged). #2160 unblocked STEP 1; this class is STEP 3.
  ...
  "com.pocketshell.app.MainActivityNonSessionRecreateTest"  # #2428 audit fix: route/back-stack/in-progress-form/in-memory-credential survival across a REAL MainActivity config-change recreate, plus the Navigator once-per-transition dedup across back-revisit + recreate. No Docker fixture, ~15s.
)
