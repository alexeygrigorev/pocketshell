...
/**
 * Ported from the old app's `com.pocketshell.app.diagnostics.DiagnosticEventJson`
 * (rewrite task P-10). Generic on-disk event encoding + redaction — the
 * connection-journal/mirror/part-store stack it used to sit alongside is NOT
 * ported (P-10 scope amendment); the one coupling this file had to that stack
 * — a reference to `ReconnectCauseTrail.CATEGORY` — is replaced below with the
 * plain string literal it resolved to, so this file has zero dependency on
...
data class DiagnosticsEvent(
    ...
    // Issue #1669 (old app): every event is version-stamped at record time so
    // the field event log is self-describing. Defaulted so historical lines /
    // synthetic test events decode without a stamp.
...
internal object DiagnosticRedactor {
     ...
     * `reconnect`). It is threaded through so a connection/reconnect cause
     * trail can SURFACE which exception a reader hit — which the blanket
     * key-based redaction would otherwise drop to `[redacted]`. Rather than
     * pass the raw exception `message` (whose free text could carry a secret a
     * caller folded in), the surfaced value is a BOUNDED, ALLOWLISTED
     * classification of the failure shape — see [classifyFailureMessage]. For
     * every other category the `message` key stays fully redacted exactly as
     * before.
     */
    ...
    private fun redactValue(key: String, value: Any?, category: String?): Any? {
        ...
        // Surface WHICH exception a diagnosable category hit so a teardown is
        // diagnosable — but as a fixed classification token, never raw message
        // text, so no secret a caller folded into the exception message can
        // ever leak.
     ...
     * classification token. This is deliberately NOT the raw message and NOT a
     * scrubbed variant of it: the return value is ALWAYS one of the fixed
     * lowercase `[a-z_]+` tokens below and NEVER a substring of [text]. That
     * gives a categorical secret-safety guarantee — an `authorization:` header,
     * a PEM private-key body, a high-entropy token, or any other secret a
     * caller folded into the exception message can never surface, because no
     * part of [text] is ever emitted. WHICH failure shape a reader hit (read-
     * timeout vs reset vs EOF vs channel-closed) is preserved; the risky free
     * text is dropped. An unrecognised shape collapses to [OTHER_FAILURE].
     */
     ...
     * reconnect diagnosis: the connection lifecycle events and a reconnect
     * cause trail. Everything else keeps `message` fully redacted.
     *
     * `"reconnect"` is a plain string literal here, not
     * `ReconnectCauseTrail.CATEGORY` — the old app's `ReconnectCauseTrail` is
     * part of the connection-journal stack this rewrite task does not port
     * (P-10 scope amendment). The literal is the exact value that constant
     * held, so behaviour is unchanged; it is just no longer a compile-time
     * coupling to a class app2 does not have.
     */
     ...
     * fingerprint via the `session` suffix — a suffix rule rather than five
     * literals so a NEW `*Session` field added tomorrow cannot silently opt out
     * of redaction.
     */
