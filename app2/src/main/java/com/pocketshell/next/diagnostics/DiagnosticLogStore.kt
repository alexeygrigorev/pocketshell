 ...
 * `com.pocketshell.app.diagnostics.DiagnosticLogStore` (rewrite task P-10).
 * A bounded ring-buffer JSONL file plus a snapshot-export helper; no coupling
 * to the connection-journal/mirror/part-store stack.
 */
internal class DiagnosticLogStore(
    ...
    fun exportSnapshot(
        ...
        // before reading the rest of the JSONL window.
 ...
 * diagnostics export. Shaped exactly like a normal diagnostics event
 * (`category`/`name`/`metadata`) so `jq`/`rg`/`tail` and the existing
 * [DiagnosticEventJson.decode] reader treat it as one more line, while
 * giving a one-`head -1` index of the exported window: event count, sequence
 * range, wall-clock range/span, per-category counts, app version, and device.
 */
