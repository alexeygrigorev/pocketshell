...
| D13 | **Chord palette** opens on long-press of ⚡ in the key bar | Named sequences (`Ctrl+B D` etc.) as tappable tiles. Per-host configurable. Many tmux chords have native UI equivalents under control mode; palette is the muscle-memory escape hatch. |
