...
#   4. exact generated copied worktrees + private Gradle home below
#      <root>/build/pre-release-confidence-gate (summaries/logs survive), and
#      <root>/build/phone-walkthrough
...
# ---------------------------------------------------------------------------
# Stage 4: transient gate scratch. Named EXPLICITLY rather than globbed under
...
#
# Issue #2055 narrows pre-release cleanup further. Each run's summary, step
# logs, and retained test XML are small and load-bearing diagnostics; keep them.
# Remove only exact <run>/worktree copied/build output plus the private
# gradle-home, and skip both while a live release owner is recorded. The helper
# makes owner-owned 0555/0444 content writable only after that exact safe-list
# check, which handles the #1714 recurrence without broad chmod/rm reach.
# ---------------------------------------------------------------------------
printf '\nStage 4: transient gate scratch under %s/build\n' "$ROOT_DIR"
gate_scratch=0
target="$ROOT_DIR/build/pre-release-confidence-gate"
if [[ -d "$target" ]]; then
  gate_scratch=$((gate_scratch + 1))
  # The lower-level retention helper refuses every unmarked root. Establish its
  # marker only after this script has anchored the target to the checkout's
  # exact canonical generated path; --root can never redirect the helper into
  # the repository source root, /var, or an arbitrary LOG_ROOT override.
  pocketshell_release_validation_prepare_root "$ROOT_DIR" "$target" || {
    printf '  REFUSING release-validation cleanup for unauthenticated root %s\n' "$target" >&2
    exit 1
  }
  if (( APPLY == 1 )); then
    pocketshell_release_validation_cleanup_all "$target" apply || {
      printf '  [APPLY]   FAILED release-validation generated-output cleanup under %s\n' "$target" >&2
      exit 1
    }
  else
    pocketshell_release_validation_cleanup_all "$target" dry-run || {
      printf '  [DRY-RUN] FAILED to inspect release-validation generated output under %s\n' "$target" >&2
      exit 1
    }
  fi
fi

target="$ROOT_DIR/build/phone-walkthrough"
if [[ -d "$target" ]]; then
  gate_scratch=$((gate_scratch + 1))
  remove_path "$target" || {
    printf '  FAILED phone-walkthrough cleanup under %s\n' "$target" >&2
    exit 1
  }
fi
