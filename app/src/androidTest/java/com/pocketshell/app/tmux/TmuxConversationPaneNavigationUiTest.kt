...
/**
 * Issue #154/#786: connected coverage for the conversation pane navigation
 * polish.
 *
 ...
 *  - **No search field (Issue #786, hard-cut D22)** — the "Search in
 *    conversation" field was DELETED to give the whole screen to the
 *    transcript. `searchFieldIsGoneAndTranscriptShowsEveryEvent` asserts the
 *    pane has no text-input field and shows every event unfiltered, and
 *    `parentedToolResultOutputMergesIntoToolCallWhenExpandedByTap` re-expresses
 *    the former search-driven tool-result-pairing proof via a tap instead.
 */
...
class TmuxConversationPaneNavigationUiTest {
    ...
    @Test
    fun parentedToolResultOutputMergesIntoToolCallWhenExpandedByTap() {
        // Issue #786: the "Search in conversation" field was hard-cut (D22), so
        // the former `searchForParentedToolResultOutputPromotesAndExpandsToolCall`
        // proof — which drove the tool-call expansion via a search query — is
        // re-expressed here WITHOUT search. The load-bearing behaviour it
        // protected (a `toolCallId`-parented ToolResult is folded into its parent
        // ToolCall card and revealed when the card is expanded by tap) survives
        // search removal because [filterConversationRows]' tool-result pairing is
        // still called (just fed an empty query).
        val events = listOf(
            ConversationEvent.ToolCall(
                id = "tool-1",
                agent = AgentKind.ClaudeCode,
        ...
        // The parented result is folded into the collapsed parent card (not a
        // standalone row) and the unrelated assistant message stays visible —
        // nothing is filtered out now that there is no query.
        compose.onNodeWithText("needle-output-only").assertDoesNotExist()
        compose.onNodeWithText("ordinary assistant text").assertExists()

        // Expand the parent ToolCall card by tap → its parented result is shown.
        compose.onNodeWithTag(TMUX_CONVERSATION_TOOL_ROW_TAG_PREFIX + "tool-1")
            .performClick()
        compose.waitForIdle()
    ...
    @Test
    fun searchFieldIsGoneAndTranscriptShowsEveryEvent() {
        // Issue #786 (hard-cut, D22): the "Search in conversation" field is
        // DELETED so the whole screen is the transcript. This replaces the old
        // `searchQueryFlowsThroughHoistedStateAndFiltersFeed`. Assert (a) no
        // search field exists in the pane, and (b) with no query every event is
        // shown (no filtering, no "No matching events." copy).
        compose.setContent {
            PocketShellTheme {
                TmuxConversationPane(
        ...
        // No search field anywhere in the conversation pane.
        compose.onAllNodes(hasSetTextAction())
            .fetchSemanticsNodes()
            .let { assertEquals(0, it.size) }

        // Every event is rendered — nothing is filtered.
