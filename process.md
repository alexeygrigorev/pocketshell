...
A red scheduled full-suite run on `main` is a feature-merge freeze: no feature/product PR merges until green (backlog work in worktrees is unaffected — only what gets merged freezes). Only a revert of the offending commit, or an already-`APPROVED` forward fix for that exact regression, may merge during a freeze. A regression bisected to a specific merge is reverted within 4 hours by default, not fixed forward while `main` stays red — after a revert, the reverted change re-enters the normal implementer→reviewer loop at full rigor. The on-call dispatched after a push owns time-to-green for the whole red period, not just the triggering push (targets: red time under 24h, merges-while-red ≈ 0). A flaking test/journey class is auto-filed as an issue, quarantined into a non-blocking lane within 24h, with a 2-week default expiry (extendable with a documented reason, not silently) so it can't be forgotten. None of this weakens D31/D32/D33. Full rationale: `docs/decisions.md` D36.

**app2 quarantine mechanism (2026-09, post-rewrite):** app2's journey lane (`app2-journey`, issue #2474) deliberately runs its whole androidTest suite in one unfiltered instrumentation process — no `-Pandroid.testInstrumentationRunnerArguments.class=` filter, ever, because a per-class filter is exactly what would hide a cross-journey pollution bug (this is how issue #2477's real bugs were found). That makes the OLD per-class quarantine shape (excluding a flaky class from a shard's runner-argument filter) structurally incompatible with app2. D36(4)'s quarantine now happens at the SOURCE level instead: `@Ignore("quarantined: #<issue>, expires <date> — <reason>")` directly on the flaky/broken `@Test` method, reconciled both directions by `scripts/check-journey-quarantine-expiry.sh` against `scripts/journey-quarantine.txt` (a method-keyed registry — every `@Ignore` needs a row, every row needs a matching `@Ignore`, an expired row hard-fails). This keeps every OTHER test in the suite running together in the same shared process (the property #2474 actually needs), while still satisfying D36(4)'s real requirement: a known-bad assertion stops blocking within its expiry instead of blocking forever. `scripts/check-app2-lane-execution.py --min N` prevents this from going vacuous — quarantining can't quietly zero out the suite.
...
**Batch-merge validation (2026-09-01, v0.4.47 postmortem):** a multi-commit batch (e.g. an "audit-fixes" sweep spanning many issues) merging straight to `main` must be validated against the SAME full gate a release needs — Docker Integration + Emulator journey + Nightly Extensive, not just the per-PR required check (Unit tests) — before it merges, not after. The v0.4.47 cycle lost most of a day because an 18-commit batch merged without that validation and silently broke `main` in five unrelated ways (a manifest gap, a shard-capacity trip, a stale CI-workflow fixture payload, a ~150-file host-key-trust regression, and a masked connection-core signal), none of which surfaced until the next release tried to cut. Per-PR checks answer "does this compile and pass fast tests"; only the full gate answers "is `main` actually releasable" — a batch this size needs the second answer before it lands, not the first.

**Same-day nightly-red triage (2026-09-01):** `full-suite-notify.yml`'s tracking issue firing on a red scheduled run is not the end of the obligation — per the quarantine SLA above, someone must act on it the same day it fires, not just when a release cut next needs `main` green. A red nightly that sits unactioned for a day is exactly how the v0.4.47 delay compounded: each individual gap was small, but none were triaged until they'd all stacked up at once.

**Bounded retry before escalating infra (2026-09-01):** when a scheduled/nightly run is red, get one full re-run and compare failure signatures before spending investigation time — a clean or differently-signed retry is G5 infra, a repeated identical signature is real. Do this once, decisively, not repeatedly re-deriving the question by hand. Automating this comparison is tracked as issue #2459 (extends `full-suite-notify.yml`); until it lands, the orchestrator/on-call does this step manually but should still do it as a single bounded action, not an open-ended investigation.

## Nightly fault-injection release gate cannot be bypassed (locked, D37)

A release tag requires a green nightly fault-injection verdict on the release commit — no code path may skip or waive this (no env var, no workflow-dispatch input, no "deliberately waived" branch). The only sanctioned way to unblock a release when the nightly is red is to fix the failing test, or to quarantine the offending class through D36(4)'s mechanism so the gate evaluates a genuinely smaller, still-real suite — never a maintainer-invoked skip of the whole verdict. Full rationale: `docs/decisions.md` D37.

**app2 cadence (2026-09, post-rewrite):** the old nightly lane ran `:app:connectedDebugAndroidTest` on a module the rewrite hard-deleted, so `app2-journey` (#2474) now carries the nightly slot directly: a `schedule: cron` trigger on `.github/workflows/app2.yml` at the same cadence, in its own concurrency group exempt from the `cancel-in-progress` a same-ref push would otherwise apply (a cancelled scheduled run reads as "not failed" while proving nothing — that would have been a silent D37 bypass). Because #2474's design runs the whole journey suite in one unfiltered pass with the fault-injection journeys inside it, the job's own conclusion IS the fault verdict; `scripts/check-nightly-fault-run.sh` reads it directly rather than synthesising one from separate phases the old multi-phase nightly script used to have.
...
- G7 (adopted) — pre-merge CI-green enforcement: `Unit tests` is the required PR check (the second check, `Python utility tests (pocketshell)`, was deleted with the host CLI's move to PocketShell-io/pocketshell-cli, issue #2643); heavy Docker/emulator jobs batch on `main` instead.
...
- Changing a shared string, content description, diagnostic event name, or a default that selects between code paths → grep the whole test tree for the old value and for oracles that depended on the path you stopped taking. The compiler won't catch this: a default flip can silently re-point an unrelated, pre-existing journey's oracle so it never fires, passing vacuously instead of reddening.
  - A default that starts being PASSED where nothing was passed before is the sub-case this rule reads over, because there is no old value to grep for: the affected call sites are the ones that OMIT the flag, and their omission was the assertion. Search for absence — call sites deliberately not passing the flag, comments explaining why one is left off — not for a stale value. `010a803f7`: #2562 made the CLI resolve a memory cap and pass `--memory` by default; #2586's fixture guard had deliberately omitted `--mem` with a comment saying a limit would fail closed in an unprivileged container. Correct when written, broken hours later. Note what a grep would actually have hit: that comment named `a start --memory` — aplexer's flag, the DOWNSTREAM failure — not `sessions create --mem`, the upstream flag whose default changed. So searching for the changed flag's own name finds nothing, and searching for the flag it was avoiding finds a comment that reads as settled reasoning rather than a dependency. Since neither name-grep works, don't lean on one: enumerate every call site of the COMMAND whose default changed (`grep -rn 'sessions create'` across the test tree, fixtures and scripts) and check each against the new default. That set is complete by construction — an omitting call site is in it precisely because it invokes the command — whereas a flag-name search is complete only over sites that already mention the flag, which is the one thing the affected sites do not do. Neither PR could catch it — `Integration tests (Docker)` is gated off on pull requests, so the interaction first ran on `main`. And the post-merge run that WOULD have caught it was cancelled (#2595 merged with its own `Tests` run killed by a later push), so `main` stayed red across two commits before anyone read it. Those are two different failures with two different fixes — a stronger PR gate, and #2600's concurrency split — and a reader who only learns the first goes looking in the wrong place. See [docs/ci-pitfalls.md](docs/ci-pitfalls.md) on a cheap push cancelling an expensive run.
- A new journey/connected test under `app2/src/androidTest/` (which the `app2-journey` lane runs wholesale, so it needs no wiring) → run it on a real emulator (`scripts/connected-test.sh --suffix i<issue>`) before merge; compiling proves it links, not that it passes under swiftshader frame timing.
...
- A hygiene-ratcheted file → run `scripts/check-file-size-hygiene.sh` locally; this downward-only guard runs in the Unit job but isn't an ordinary Gradle test. When a batch of PRs shares the file, re-check against the actual post-merge `main`, not just per-PR.
...
Monitor the cheap required checks, but don't treat waiting as the main activity — keep triaging, dispatching, reviewing. Heavy Docker/emulator jobs batch on `main` pushes and manual dispatch, not per-PR; since D40 (issue #2593) the `main` concurrency group does NOT cancel superseded push runs — every push run completes and validates exactly its own diff, so back-to-back merges queue their heavy lanes serially instead of validating "a batch at the newest head" (which is how a cancelled app2 merge reached `main` with zero journey execution: #2593). Merge compatible, already-approved small slices one by one, and expect each to queue its own run rather than fold into the next (the account has ~20 concurrent Actions jobs; a full `Tests` run occupies ~4, so ~5 PRs with running Actions saturate it). Never merge onto a `main` a scheduled full-suite run has marked red (D36) — only a release cut, red-CI investigation, or a direct dependency on that exact pipeline justifies blocking on it. If CI fails despite a green local gate, that's a process miss: add the local check that would have caught it.

### Protected `main` checks

`main` requires PR-based merges for meaningful feature/code/risky slices, with this exact required `Tests` workflow check: `Unit tests` (the host CLI's own CI lives in PocketShell-io/pocketshell-cli since issue #2643). Heavy `Integration tests (Docker)` and the emulator journey lane — `app2 journey suite (emulator + Docker agents)`, which moved to the `app2` workflow in the rewrite — batch on `main`/manual dispatch, not per-PR; `scripts/watch-ci.py` covers the `Tests` pair by default and needs an explicit `--required-check "app2 journey suite (emulator + Docker agents)"` for the journey lane (issue #2545). GitHub's strict "branch must be up to date" stays off (a docs-only push would otherwise force every open PR through another full Docker/emulator run) — the orchestrator rebases a PR manually when intervening `main` commits changed overlapping code, workflow behavior, or dependencies, and inspects a red/cancelled check before ever rerunning it. Trivial/docs-only direct-to-main commits don't need these checks and must not queue emulator CI. An admin/emergency bypass may exist for a solo-maintainer deadlock; any bypassed push is documented on the issue with the reason and the follow-up run that restored green.
